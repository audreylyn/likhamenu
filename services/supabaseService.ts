...
// Note: Supabase client will handle missing URL/key gracefully, but operations will fail
...
export const uploadImage = async (file: File, bucket = IMAGE_BUCKET, path?: string) => {
  try {
    ...
    if (!compressedFile) {
      throw new Error('Image compression failed: The compressed file is missing');
    }

    // Ensure we have a File object (not just Blob) for Supabase
    const fileToUpload = compressedFile instanceof File 
      ? compressedFile 
      : new File([compressedFile], file.name || 'image.jpg', { type: (compressedFile as Blob).type || 'image/jpeg' });
...
export const saveWebsite = async (website: Website) => {
  ...
  try {
    ...
    // Also check for status 406 in case code is different
    const is406 = updateError && (
      updateError.code === '406' || 
      (updateError as any).status === 406 ||
      updateError.message?.includes('Not Acceptable')
    );

    if (updateError && updateError.code !== 'PGRST116' && !is406) {
